...
"""Make calculations for a task of SOR, corrected"""
...
if __name__ == "__main__":
    ...
    omega = 2.0 / (max((abs(val) for val in eigenA)) + min((abs(val) for val in eigenA)))

    matB = omega * matL + matD
    matC = (omega - 1.0) * matD + omega * matU
    matB_inv = np.linalg.inv(matB)
    matG = np.matmul(-matB_inv, matC)
    vec_g = np.matmul(matB_inv, omega * vec_b)
